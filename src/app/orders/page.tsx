...
export default async function OrdersPage() {
  ...
  const buyerIds = [
    ...new Set(ordersData.map((o) => o.buyer_id).filter(Boolean)),
  ];
  ...
  const enrichedOrders = ordersData.map((order) => {
    ...
    const calculatedTotalPrice = items.reduce((sum, item) => {
      const unitPrice =
        item.product_price_per_unit || item.price_at_purchase || 0;
  ...
  const totalRevenue = enrichedOrders.reduce(
