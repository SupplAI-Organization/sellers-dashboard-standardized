...
export default async function DashboardPage() {
  ...
  const displayName =
    userDetail?.contact_person || supplierProfile?.contact_person || "Seller";
  const displayEmail = user.email || "";

  // Calculate top-selling products from order items
  const productSales = new Map<
    string,
    { quantity: number; revenue: number; productData?: (typeof products)[0] }
  >();
