...
export default async function MyProductsPage() {
  ...
  const { data: suppProfiles } = await supabase
    ...
    .eq("supplier_id", user.id);
  const displayName =
    userDetails?.[0]?.contact_person ||
    suppProfiles?.[0]?.contact_person ||
    "Seller";
  const displayEmail = user.email || "";

  return (
    <div className="flex flex-col h-screen bg-white overflow-hidden w-full">
      <AppTopbar userName={displayName} userEmail={displayEmail} />
      <div className="flex flex-1 overflow-hidden w-full">
        <AppSidebar />
        <div className="flex-1 overflow-auto space-y-6 py-8 px-4 md:px-8 bg-slate-50">
          {/* Header */}
            ...
            <Card style={{ backgroundColor: "#ffffff" }} className="shadow-sm">
              <CardContent className="pt-6">
                <div
                  ...
                  style={{ color: "#1B3C53" }}
                >
                  {products?.filter((p) => p.availability_status === "in_stock")
                    .length ?? 0}
                </div>
                <p className="text-xs mt-1" style={{ color: "#666" }}>
                  In Stock
          ...
          <Card style={{ backgroundColor: "#ffffff", borderColor: "#D2C1B6" }}>
            <CardHeader
              style={{
                ...
                borderBottomColor: "#D2C1B6",
              }}
              className="border-b"
            >
              <CardTitle style={{ color: "#1B3C53" }}>
                Product Inventory
              </CardTitle>
            </CardHeader>
